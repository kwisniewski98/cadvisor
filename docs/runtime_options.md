...
--disable_metrics=tcp,advtcp,udp,sched,process,hugetlb: comma-separated list of metrics to be disabled. Options are 'disk', 'network', 'tcp', 'advtcp', 'udp', 'sched', 'process', 'hugetlb', 'numa_vmstat'. Note: tcp and udp are disabled by default due to high CPU usage. (default tcp,advtcp,udp,sched,process,hugetlb,numa_vmstat)
...
```
-numa_vmstats_metrics: Regular expression to filter /proc/vmstat metrics. (default ".*")
```
I.e. `-numa_vmstat_metrics="workingset.*"` will show only metrics starting with workingset word.
